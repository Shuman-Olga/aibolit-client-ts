import { IPDF } from "../types/IPDF";

export const DataPravoInfa: IPDF[] = [
  // { title: "Правила внутреннего распорядка для пациентов", pdf: "dfgd", infa: "" },
  {
    title:
      "Программа государственных гарантий бесплатного оказания медицинской помощи",
    pdf: "programma_gosudarstvennyh_garantij_besplatnogo_okazaniya_medicinskoj_pomoshchi.pdf",
  },
  {
    title: "Территориальная программа госгарантий",
    pdf: "programma_2024.pdf",
  },
  {
    title: "Лекарственное обеспечение",
    pdf: "perechen_zhiznenno_neobhodimyh_i_vazhnejshih_lekarstvennyh_preparatov_dlya_medicinskogo_primeneniya_2024.pdf",
  },
  {
    title: "Права и обязанности граждан в сфере охраны здоровья",
    pdf: "prava-i-obyazannosti-grazhdan-v-sfere-ohrany-zdorovya.pdf",
  },
  {
    title: "Правила подготовки к диагностическим исследованиям",
    pdf: "pravila_podgotovki_k_diagnosticheskim_issledovaniyam.pdf",
  },
  {
    title:
      "Информация о методах оказания медицинской помощи, связанных с ними рисках; видах медицинского вмешательства, их последствиях и ожидаемых результатах оказания медицинской помощи",
    pdf: "inf-o-metodah-okaz-medpom-svyazannyh-s-nimi-riskah-vidah-med-vmeshat-posledstviyah-i-dr.pdf",
  },
  {
    title:
      "Перечень категорий потребителей, имеющих право на получение льгот, а также перечень льгот, предоставляемых при оказании платных медицинских услуг",
    pdf: "7-perechen-kategorij-potrebitelej-imeyushchih-pravo-na-poluchenie-lgot.pdf",
  },
  {
    title:
      "Стандарты и порядки оказания медицинской помощи, клинические рекомендации",
    pdf: "perechen_standartov_medicinskoj_pomoshchi_bolnym_pri_razlichnyh_zabolevaniyah.pdf",
  },
  {
    title: "Сроки ожидания предоставления платных медицинских услуг",
    pdf: "sroki_ozhidaniya_predostavleniya_platnyh_medicinskih_uslug.pdf",
  },
  {
    title: "Правила предоставление платных медицинских услуг ",
    pdf: "736medkadry.pdf",
  },
];
